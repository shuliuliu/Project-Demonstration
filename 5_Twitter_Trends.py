...
def find_centroid(polygon):
    ...
    area=0
    lat,lon,plus,minus = 0,0,0,0

    if n>3:
        for j in range(n-1):
            plus += latitude(polygon[j])*longitude(polygon[j+1])
            minus += latitude(polygon[j+1])*longitude(polygon[j])
        temp = 0.5*(plus+latitude(polygon[n-1])*longitude(polygon[0])-minus-latitude(polygon[0])*longitude(polygon[n-1]))
        area = abs(temp)
    if round(area,5) != 0.0:
        for i in range(n-1):
...
def find_state_center(polygons):
    ...
    for poly in polygons:
        lat,lon,area = find_centroid(poly)
        lat_weight += lat*area
        lon_weight += lon*area
        total_area += area
